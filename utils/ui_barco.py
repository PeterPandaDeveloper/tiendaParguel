...
EMOTE = "<:ricomacpato:AQUI_PONES_EL_ID>" # Pon aquí el ID de tu emoji
...
class SelectorCompra(discord.ui.Select):
    def __init__(self, archivo_tienda):
        ...
        opciones = []
        for obj in self.datos_tienda['objetos']:
            if obj['stock_actual'] > 0:
                opciones.append(discord.SelectOption(
                    label=f"{obj['nombre']} ({obj['precio']} MR)",
                    description=f"Stock: {obj['stock_actual']} | {obj['description'][:40]}...",
                    value=obj['id']
                ))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        obj_comprar = next((o for o in self.datos_tienda['objetos'] if o['id'] == item_id), None)
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            if saldo < obj_comprar['precio']:
                return await interaction.response.send_message(f"❌ Fondos insuficientes. Cuesta **{obj_comprar['precio']}** MR y tienes **{saldo}** {EMOTE}.", ephemeral=True)
            ...
            if item_id == "restock_tienda":
                # Devolvemos todos los ítems de esta tienda a su stock máximo
                for obj in self.datos_tienda['objetos']:
                    obj['stock_actual'] = obj['stock_maximo']
                
                await db.commit()
                
                # Guardamos los cambios inmediatamente en el JSON
                ...
                return await interaction.response.send_message(f"⚡ {interaction.user.mention} ha comprado un **RESTOCK DE TIENDA** por **600** MR.\n🌿 *Las enredaderas de El Brote Hundido se agitan violentamente y la bodega se ha reabastecido por completo al Máximo.*")

            # Lógica normal para cualquier otro ítem (Añadir a inventario)
        ...
        # Reducir stock normal del ítem comprado y guardar
        obj_comprar['stock_actual'] -= 1
        ruta = os.path.join(COMPARTIMIENTOS_PATH, self.archivo_tienda)
        with open(ruta, 'w', encoding='utf-8') as f:
            json.dump(self.datos_tienda, f, indent=4, ensure_ascii=False)
            
        await interaction.response.send_message(f"🎉 ¡Has adquirido **{obj_comprar['nombre']}** por **{obj_comprar['precio']}** MR!\n📦 Guardado en tu `/pt-inventario`.", ephemeral=False)
